...
// ---- setup builder --------------------------------------------------------
function buildTakeProfits(mid: number, profile: ScannerProfile, rng: () => number): TakeProfit[] {
  ...
  const rows: ChainRow[] = window.map(node => {
    ...
    return {
      ...
      call: {
        ...
        changePct: Math.round(clamp((spot - node.strike) / spot * 800 + callRng() * 30, 1, 130)),
        health: callHealth,
      ...
      put: {
        ...
        changePct: Math.round(clamp((node.strike - spot) / spot * 800 + putRng() * 30, 1, 130)),
        health: putHealth,
